...
type CustomValidation = (data: any) => string | undefined | boolean
  ...
  if (Array.isArray(type)) {
    ...
    if (typeof data[propertyName] !== type) {
      if (typeof data[propertyName] === 'string') {
        const newNumberValue: number = +(data[propertyName] as string)
        if (isNaN(newNumberValue)) {
          ...
          data[propertyName] = newNumberValue
        }
      } else {
        if (throwInterfaceValidationErrors()) {
          throw new Error(
...
export const validateCustom = (
  ...
  if (type === 'map') {
    if (custom !== undefined) {
      ...
      const result: string | undefined | boolean = newCustom(data[propertyName])
